...
3. Copia `RenzoAgostini.Server/appsettings.Production.template.json` in `RenzoAgostini.Server/appsettings.json`
   ...
   tutto tramite secrets di Fly o, per lo sviluppo locale, con `dotnet user-secrets` (supportato automaticamente
   dall'app grazie alla chiamata a `AddUserSecrets`). In particolare verifica:
   ...
   Per usare i secrets locali puoi, ad esempio, eseguire:

   ```bash
   cd RenzoAgostini.Server
   dotnet user-secrets set "ConnectionStrings:DefaultConnection" "Data Source=RenzoAgostini.db"
   dotnet user-secrets set "Keycloak:Authority" "https://auth.local/realms/RenzoAgostiniRealm"
   dotnet user-secrets set "Stripe:SecretKey" "sk_test_123"
   ```
